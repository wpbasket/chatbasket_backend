...
// *core_auth.AuthService satisfies this interface directly — no adapter needed.
...
func (s *chatService) CheckMessagingEligibility(ctx context.Context, senderID, recipientID uuid.UUID) (string, error) {
	// 1. Check recipient exists + profile_type (profile-owned — delegated to ProfileProvider)
	...
	// 2. Admin-block checks (profile-owned — delegated to ProfileProvider)
	...
	// 3. Contact / block checks (same-schema tables — CanSendMessageLite)
	...
	// 4. Primary-device check (auth-owned — AuthProvider)
...
func (s *chatService) GetUserChatsHandler(ctx context.Context, userID uuid.UUID) (*GetUserChatsResponse, error) {
	...
	// OtherUserID is interface{} from sqlc (CASE expression) — assert to uuid.UUID
